...
- Reuse `dataAdapter`, `storageAdapter`, `realtimeAdapter`, `functionAdapter`, and `notificationAdapter` rather than creating Appwrite calls inside features.
...
- Do not hard-code product database IDs, table IDs, bucket IDs, function IDs, or product permissions into core.
- Permission prompts such as notifications should be user-triggered, not automatically shown on startup.
...
For native-facing changes, also run the app in an Expo development build on Android and iOS when an appropriate environment is available.
